...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {		
					...
					if (Duration.between(from, today).toDays() > 0) {
						txtValor.setForeground(Color.RED);
						txtValor.setText("Entrada deve ser a partir de hoje");
						
						ReservasView.txtDataE.setDate(null);
						ReservasView.txtDataS.setDate(null); 
					} else if (d.toDays() > 0) {
						
						txtValor.setForeground(Color.BLACK);
						txtValor.setText("R$ " + reservasDAO.calcReserva(from, to, formaPagamento));
						
					} else {
						txtValor.setForeground(Color.RED);
						txtValor.setText("Saída deve ser depois da Entrada");
		...
		txtValor.setForeground(Color.BLACK);
		txtValor.setBounds(68, 328, 290, 33);
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {	
					LocalDateTime from = LocalDateTime.ofInstant(txtDataE.getDate().toInstant(), ZoneId.systemDefault());
					LocalDateTime to = LocalDateTime.ofInstant(txtDataS.getDate().toInstant(), ZoneId.systemDefault());
					double valorDaReserva = Double.parseDouble(txtValor.getText().replace("R$ ", ""));
					
					try {
						reservasDAO.saveReserva();
					} catch(Exception e1) {
						e1.printStackTrace();
					}
					
					RegistroHospede registro = new RegistroHospede();
					registro.setReservaNumber(reservasDAO.getReservaNumber());
