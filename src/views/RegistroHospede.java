...
public class RegistroHospede extends JFrame {
	...
	public RegistroHospede() {
		...
		btnexit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				int dialogResult = JOptionPane.showConfirmDialog(null, "Essa ação cancelará a reserva. Deseja continuar?","Warning",JOptionPane.YES_NO_OPTION);
				if(dialogResult == JOptionPane.YES_OPTION){
					try {
						ReservasDAO reservasDAO = new ReservasDAO();
						reservasDAO.cancelReserva(Integer.parseInt(txtNreserva.getText()));
					} catch(Exception e2) {
						e2.printStackTrace();
					}
		...
		btnsalvar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String surname = txtSobrenome.getText();
				java.sql.Date birthDate = new java.sql.Date(txtDataN.getDate().getTime());
				String nacionality = txtNacionalidade.getSelectedItem().toString();
				String telephone = txtTelefone.getText();
				int idReserva = Integer.parseInt(txtNreserva.getText());
				
				if (RegistroHospede.txtDataN.getDate() != null && RegistroHospede.txtNome.getText() != null 
						&& RegistroHospede.txtSobrenome.getText() != null && RegistroHospede.txtTelefone.getText() != null) {	
					try {
						
						HospedesDAO hospedesDAO = new HospedesDAO();
						hospedesDAO.saveHospede(name, surname, birthDate, nacionality, telephone, idReserva);
						
						Sucesso sucesso = new Sucesso(String.valueOf(idReserva));
						sucesso.setVisible(true);
						
						dispose();
						
					} catch(Exception e1) {
						JOptionPane.showMessageDialog(null, "Não foi possível confirmar a reserva!");
						e1.printStackTrace();
					}
				} else {
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" y "y"
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	    ...
	    public void setReservaNumber(String reservaNumber) {
			this.reservaNumber = reservaNumber;
			
			// Coloca o provável número de reserva na tela
			txtNreserva.setText(this.reservaNumber);
		}
